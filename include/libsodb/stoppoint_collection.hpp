...
auto stoppoint_collection<Stoppoint>::find_by_id(typename Stoppoint::id_type id) ->
    ...
    return std::ranges::find_if(stoppoints_, [=](auto& point) { return point->id() == id; });
}
...
template <class Stoppoint>
bool stoppoint_collection<Stoppoint>::enable_stoppoint_at_address(virt_addr address) const {
